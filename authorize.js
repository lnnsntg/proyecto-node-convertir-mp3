...
async function authorize(credentials, nameFile, title) {
  const clientSecret = await credentials.installed.client_secret;
  const clientId = await credentials.installed.client_id;
  const redirectUrl = await credentials.installed.redirect_uris[0];
  const oAuth2Client = new OAuth2(clientId, clientSecret, redirectUrl);

  // Check if we have previously stored a token.
  fs.readFile(TOKEN_PATH, async (error, token) => {
    if (error) {
      return getNewToken(oAuth2Client, nameFile, title);
    } else {
      let tokens = await JSON.parse(token);
...
const getNewToken = async (oAuth2Client, nameFile, title) => {
  // Aqui cb es la llamada al servicio subir video
